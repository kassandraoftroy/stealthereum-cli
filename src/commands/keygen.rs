...
pub fn run(keystore: Option<PathBuf>, chain_id: Option<u64>) -> std::io::Result<()> {
    ...
    if path.join(SECRET_KEY_FILENAME).exists()
        || path.join(VIEWING_KEY_FILENAME).exists()
        || path.join(ENCRYPTED_LOGS_FILENAME).exists()
    {
    ...
    println!(
        "stored files:\n{}\n{}\n{}\n",
        path.join(SECRET_KEY_FILENAME).display(),
...
fn create_password() -> String {
    ...
    let confirm_password =
        prompt_password("Confirm stealthereum password:").expect("Failed to read password");
