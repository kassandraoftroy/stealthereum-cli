...
use crate::utils::{
    ...
    Logfile,
    scan_for_payments,
    store_encrypted_logfile,
};
use crate::constants::{ANNOUNCER_ADDRESS, DEFAULT_KEYSTORE_DIR, ENCRYPTED_LOGS_FILENAME, get_default_chain_id, get_default_starting_block, get_default_rpc};
...
pub async fn run(
    ...
    let (payments, latest_block) = scan_for_payments(&rpc, &ANNOUNCER_ADDRESS.to_string(), &logfile.latest_block, sk, vk).await;
    println!("Found {} new payments, storing to keystore...", payments.len());
